-- =============================================================
-- 招投标信息推送系统 · PostgreSQL / Supabase 建表脚本（001 · 全量基线）
--
-- 来源：由 docs/初始化.sql 归位而来，本文件为**唯一维护副本**；
--       docs/初始化.sql 保留为历史副本，不再修改（当前工作区无版本控制，故不做物理删除）。
-- 依据：docs/系统设计.md + docs/architecture.md §4
...
--         脚本内所有 DDL 均带 IF NOT EXISTS / OR REPLACE，可重复执行
--
-- ⚠️ 表结构变更请新建 003_xxx.sql，不要改本文件（本文件是基线，改动会导致新旧环境结构漂移）。
-- ⚠️ 本文件不启用 RLS；行级安全策略见 002_app_fields_migration.sql。
-- =============================================================
...
-- 种子订阅：空条件 = 订阅全部公告。
-- 原脚本为无条件 INSERT，重复执行会产生重复订阅；此处按 webhook_url 幂等化（保持「可重复执行」）。
